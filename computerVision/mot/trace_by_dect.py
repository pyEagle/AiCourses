...
    SLEEP = "休眠 (抽屉已关闭)"  
    COVERED = "被遮挡/堆叠 (Covered-Hidden)" 
    HIDDEN_BY_CABINET = "藏于柜内 (Hidden-HalfOpen)"
    PENDING_DELETE = "待删除确认 (Pending-Delete)"
...
        
        # 【完美修复 4】：注入先验种子，防止全静止冷启动导致三个高斯分布粘合
...
    print("\n========= 【最终完美版：工业级 ReID + IoU 追踪器验证】 =========")
    
    # 测试 1: 翻面 (Flipping) 特征画廊兼容性测试
    print("\n--- 测试 1: 物品原地翻面 (Feature Gallery) 测试 ---")
...
    print(f"  放入时 Track ID: {tracker.tracks[0].track_id}, 画廊容量: {len(tracker.tracks[0].gallery.gallery)}")
    
    # 物品在极短时间内原地翻转（由于空间马氏距离极小，虽然外观变了，但仍能强行关联匹配）
    # 在真实世界中，用户手翻动会经历一个连续过程，这里模拟最突变的极端情况
