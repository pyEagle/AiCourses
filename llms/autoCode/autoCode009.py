    ...
    def run(self, spec):
        ...
        try:
            ...
            for file_info in spec["files"]:
                ...
                if (
                        "code" not in file_info or
                        "pyfile" not in file_info
                ):
                    return False, "文件字段缺失"
...
class ResearchAgent:
    ...
    def _build_context(self, last_log):
        ...
        cluster = self.bandit.get_task_cluster(
            self.task
        )
...
"""
    ...
    def run(
        ...
        for round_id in range(iterations):
            ...
            candidates = self.bandit.retrieve_similar_arms(
                base_prompt,
                cluster,
                topk=5
            )
            ...
            for fix_step in range(max_fixes + 1):
                ...
                success, last_log = self.runner.run(
                    spec
                )
